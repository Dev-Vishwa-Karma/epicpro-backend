...
function insertConnectUsers($conn, $connect_id, $employees, $created_at, $updated_at, $pusher, $title, $message, $config, $status, $type) {
    ...
    $stmt = $conn->prepare("
        ...
        (connect_id, employee_id, connect_status, created_at, updated_at) 
        VALUES (?, ?, 'Unread', ?, ?)
    ");
    $receiver = [];
    $errors = [];
    ...
    switch ($action) {
        ...
        case 'get_connects':
            ...
            if($filter === 'sent' || $filter === 'draft'){
                $query = "
                    ...
                    LEFT JOIN connects_users ne ON ne.connect_id = pn.id
                ...
                $query = "
                    ...
                    LEFT JOIN connects pn ON pn.id = nu.connect_id 
                    LEFT JOIN employees e ON e.id = pn.created_by
        ...
        case 'add':
            ...
            try {
                ...
                $connect_id = saveConnects($conn, $data, $data['id']);
                if ($data['id']) {
        ...
        case 'mark_read':
            if (isset($_GET['user_id']) && is_numeric($_GET['user_id'])) {
                ...
                if (isset($_GET['connect_id']) && is_numeric($_GET['connect_id'])) {
                ...
                if ($stmt->execute()) {
                    sendJsonResponse('success', null, 'Connect(s) marked as read');
                } else {
                    sendJsonResponse('error', null, 'Failed to mark Connects as read');
                }
            } else {
        ...
        case 'update_status':
            ...
            $stmt = $conn->prepare("
                UPDATE connects_users 
                SET connect_status = ? 
                WHERE connect_id = ? AND employee_id = ?
            ");
