...
if (isset($action)) {
    switch ($action) {
        case 'check-login':
            ...
            $stmt = $conn->prepare("SELECT id, first_name, last_name, email, role, dob, gender, status, profile, public_key, deleted_at FROM employees WHERE email = ? AND password = ? ORDER BY id DESC LIMIT 1");
